    ...
    <div className="flex min-h-screen font-sans">
      ...
      <main className="flex-1 overflow-auto">
        <div className="px-8 py-8 max-w-3xl">
          {/* Back */}
          <button onClick={() => navigate(`/jobs/${jobId}`)} className="flex items-center gap-1 text-xs text-white/30 hover:text-white/50 mb-6 transition-colors">
          ...
          <div
            className={`animate-fade-up border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-200 mb-6 ${
              isDragging
                ? 'border-cyan-400 bg-indigo-50 scale-[1.01] shadow-lg shadow-indigo-100'
                : 'border-white/[0.07] bg-white hover:border-cyan-300 hover:bg-indigo-50/50'
            }`}
          ...
          >
            ...
            />
            <div className={`w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center transition-all duration-200 ${isDragging ? 'bg-indigo-100 scale-110' : 'bg-white/[0.04]'}`}>
              <svg className={`w-8 h-8 transition-colors duration-200 ${isDragging ? 'text-cyan-500' : 'text-white/30'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
            <p className="text-white/70 font-medium mb-1">Drop PDF or DOCX files here</p>
            <p className="text-sm text-cyan-500 font-medium mb-2">or click to browse from your computer</p>
            <p className="text-xs text-white/30">Max file size: 10MB per document</p>
          </div>

          {/* Processing Queue */}
          {queue.length > 0 && (
            <div className="animate-fade-up rounded-2xl border border-white/[0.07] bg-white/[0.03] overflow-hidden shadow-sm" style={{ animationDelay: '120ms' }}>
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.05]">
                <h2 className="font-semibold text-white">Processing Queue</h2>
                <span className="text-xs bg-indigo-100 text-cyan-700 font-bold px-2.5 py-1 rounded-full">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-medium text-white truncate">{qf.file.name}</span>
                            <div className="flex items-center gap-2 ml-3 flex-shrink-0">
                              {isDone && qf.score !== null && (
                                <span className={`text-sm font-bold tabular-nums animate-score-pop ${qf.score >= 80 ? 'text-green-600' : qf.score >= 60 ? 'text-yellow-600' : 'text-red-500'}`}>
                                  {Math.round(qf.score)}
                                  <span className="text-xs font-normal text-white/30 ml-0.5">Match Score</span>
                                </span>
                              )}
                              {isErr && <span className="text-xs text-red-500">{qf.errorMsg}</span>}
                              <button
                                onClick={() => removeFile(qf.id)}
                                className="w-6 h-6 flex items-center justify-center text-gray-300 hover:text-white/40 transition-colors"
                              >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs ${isDone ? 'text-green-600 font-medium' : isErr ? 'text-red-500' : 'text-white/30'}`}>
                            ...
                            {!isDone && !isErr && (
                              <>
                                ...
                                <span className="text-xs text-white/30 tabular-nums w-8 text-right">{qf.progress}%</span>
                              </>
                            )}
              ...
              {/* Footer actions */}
              <div className="flex items-center justify-between px-5 py-4 border-t border-white/[0.05] bg-white/[0.02]/60">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="text-sm text-white/50 border border-white/[0.07] hover:bg-white/[0.04] px-4 py-2 rounded-lg font-medium transition-colors"
                  >
                    Cancel
                  </button>
                  <button
